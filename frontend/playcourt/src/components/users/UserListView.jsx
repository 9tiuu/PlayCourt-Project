...
const UserListView = () =>{
    ...
    // Actualizar y eliminar pendientes***
    ...
    const addNewUser = async (e) => {
        ...
        if (formData.name.trim() === '' || formData.lastname.trim() === '' || formData.email.trim() === '' || !formData.gender || !formData.rol) {
                            ...
                            listUsers.map((user, index) => (
                                <tr key={index} className="transition-colors duration-150 text-sm">
                                    ...
                                    <td className="px-6 py-3">
                                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${user.rol?.namerol === "Administrador" ? "bg-blue-200 text-blue-800" : "bg-orange-100 text-orange-700"}`}>
                                            {user.rol?.namerol}
                                        </span>
                                    </td>

                                    {
                                        user.rol?.namerol === 'Administrador' ? (
                                            ...
                                            <td className='px-6 py-3 flex gap-2'>
                                                ...
                                                <button onClick={ActivateDeleteUserModal} className="bg-red-500 hover:bg-red-600 hover:duration-300 duration-300 hover:scale-[105%] transition hover:transition text-white p-2 rounded" title="Eliminar">
                ...
                deleteUserModal && (
                    <div className="absolute flex items-center justify-center top-0 left-0 bottom-0 right-0 w-full h-full bg-black bg-opacity-45 p-4">
                        <div className="bg-white dark:bg-color2 md:w-[40rem] w-full h-auto rounded-lg p-6">
                            ...
                            <div className="flex gap-4 md:items-center justify-end mb-6 md:flex-row flex-col">
                                <button onClick={ActivateDeleteUserModal} className="bg-red-500 flex items-center justify-center gap-2 hover:bg-red-600 hover:duration-300 duration-300 hover:scale-[105%] transition hover:transition text-white py-3 px-6 rounded" title="Cancelar">
                                    Cancelar
                                </button>
